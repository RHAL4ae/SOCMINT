...
class AuthService {
  ...
  static Future<String?> getUserNameFromToken() async {
    ...
    final payload = JwtUtils.decodeJwtPayload(token);
    if (payload == null) return null;
    // Assuming the claim for username is 'name' or 'username'
  ...
  static Future<String?> getNationalIdFromToken() async {
    final token = await ApiService.getToken();
    if (token == null) return null;
    final payload = JwtUtils.decodeJwtPayload(token);
    if (payload == null) return null;
    // Assuming the claim for national ID is 'national_id' or 'sub'
    if (payload.containsKey('national_id')) {
      return payload['national_id'] as String?;
    } else if (payload.containsKey('sub')) { // 'sub' is a common claim for subject/user ID
